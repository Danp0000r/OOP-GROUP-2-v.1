...
class BuildFixServiceTestCase(unittest.TestCase):
    ...
    def test_fix_build_returns_fixed_when_already_compatible(self):
        # Compatible builds should be reported as fixed without changes.
        parts = [{"category": "CPU", "name": "Test CPU"}]
        report = {"compatible": True, "status": "compatible"}

        with patch(
            "services.build_finder.build_fix_service.CompatibilityService.evaluate_build",
            return_value=report,
        ) as mocked_eval:
    ...
    def test_fix_build_returns_false_when_incompatible_and_no_fix_possible(self):
        # Incompatible builds with no fix available should return fixed=False.
        parts = [
            {
                "category": "CPU",
                "name": "Test CPU",
        ...
        with patch(
            "services.build_finder.build_fix_service.CompatibilityService.evaluate_build",
            return_value=incompatible_report,
        ), patch(
            "services.build_finder.build_fix_service.Component", new=fake_component
        ):
            result = BuildFixService.fix_build(parts)
...
class QuestionnaireServiceTestCase(unittest.TestCase):
    ...
    def test_get_build_recommendation_returns_issue_when_no_builds(self):
        # Questionnaire recommendation should report issues when no builds are found.
        answers = {"targetTier": "Gaming", "budget": "Normal Cost"}
        with patch(
            "services.build_finder.questionnaire_service._generate_all_candidate_builds",
            return_value=[],
        ):
    ...
    def test_get_build_recommendation_selects_build_and_returns_metadata(self):
        # Recommendation should select a compatible build and return its metadata.
        answers = {
            "targetTier": "Gaming",
            "budget": "High Cost",
            "platform": "Team Intel & DDR5 Memory",
        }
        simple_build = {
            "CPU": {
                "id": 1,
        ...
        with patch(
            "services.build_finder.questionnaire_service._generate_all_candidate_builds",
            return_value=[simple_build],
        ), patch(
            "services.build_finder.questionnaire_service._categorize_builds_by_price",
            return_value={"low": [], "normal": [], "high": [simple_build]},
        ), patch(
            "services.build_finder.questionnaire_service.CompatibilityService.evaluate_build",
            return_value=evaluated_report,
        ), patch(
            "services.build_finder.questionnaire_service.BuildFixService.fix_build",
            return_value={
                "fixed": False,
                "components": [],
                "changes": [],
                "compatibility_report": evaluated_report,
            },
        ):
            result = get_build_recommendation(answers)
    ...
    def test_get_build_recommendation_uses_fix_when_initial_build_incompatible(self):
        # Incompatible initial builds should be repaired using the build fixer.
        answers = {
            "targetTier": "Gaming",
            "budget": "Normal Cost",
            "platform": "No Preference (Optimized Value)",
        }
        initial_build = {
            "CPU": {
                "id": 1,
        ...
        with patch(
            "services.build_finder.questionnaire_service._generate_all_candidate_builds",
            return_value=[initial_build],
        ), patch(
            "services.build_finder.questionnaire_service._categorize_builds_by_price",
            return_value={"low": [], "normal": [initial_build], "high": []},
        ), patch(
            "services.build_finder.questionnaire_service.CompatibilityService.evaluate_build",
            side_effect=[incompatible_report, fixed_report],
        ), patch(
            "services.build_finder.questionnaire_service.BuildFixService.fix_build",
            return_value={
                "fixed": True,
                "components": fixed_build,
                "changes": [],
                "compatibility_report": fixed_report,
            },
        ):
            result = get_build_recommendation(answers)
