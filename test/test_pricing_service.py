...
class PricingServiceTestCase(unittest.TestCase):
    ...
    def test_calculate_build_total_sums_component_prices(self):
        # calculate_build_total should sum prices and return the correct breakdown.
        comp1 = MagicMock()
        comp1.id = 1
        comp1.name = "Part1"
        comp1.category = "CPU"
    ...
    def test_get_store_prices_returns_cheapest_store_list(self):
        # get_store_prices should sort stores and return the cheapest option.
    ...
    def test_cheapest_build_uses_store_prices_and_fallback(self):
        # cheapest_build should use store pricing and fallback to part price when needed.
        comp = MagicMock(name="Part1", price=120)
        fake_component = MagicMock()
        fake_component.query.get.return_value = comp
        with patch(
            "services.pricing.pricing_service.get_store_prices",
            return_value={
                "cheapest": {"store": "StoreA", "price": 120, "url": "http://a"},
                "stores": [{"store": "StoreA", "price": 120, "url": "http://a"}],
            },
    ...
    def test_price_budget_check_reports_under_and_over(self):
        # price_budget_check should correctly flag builds that are under or over budget.
        with patch(
            "services.pricing.pricing_service.calculate_build_total",
            return_value={"total": 100, "breakdown": [{"price": 100}], "count": 1},
        ):
