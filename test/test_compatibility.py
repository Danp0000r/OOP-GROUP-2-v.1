...
class CompatibilityServiceTests(unittest.TestCase):
    ...
    def test_loader_preserves_performance_scores(self):
        ...
        self.assertEqual(
            rx7700.get("performance_score"), 75, "RX 7700 XT score should be 75"
        )
    ...
    def test_unknown_components_do_not_block_analysis(self):
        ...
        self.assertTrue(
            report["compatible"], "build should still be compatible with unknown part"
        )
        self.assertEqual(
            report["status"], "warning", "status should be warning for unknown items"
        )
        self.assertTrue(
            any(
        ...
        self.assertTrue(
            any(part.get("source") == "unknown" for part in report["detectedParts"]),
            "unknown source should be marked",
        )
    ...
    def test_socket_mismatch(self):
        ...
        self.assertEqual(
            report["status"], "incompatible", "status should show incompatible"
        )
        self.assertTrue(
            any("Socket mismatch" in issue["message"] for issue in report["issues"]),
            "issue should mention socket mismatch",
        )
    ...
    def test_empty_build(self):
        ...
        self.assertEqual(
            report["status"], "compatible", "empty build should be compatible"
        )
    ...
    def test_ram_type_mismatch(self):
        ...
        self.assertEqual(
            report["status"], "incompatible", "status should say incompatible"
        )
        self.assertTrue(
            any("RAM type mismatch" in issue["message"] for issue in report["issues"]),
            "should report a RAM type mismatch",
        )
    ...
    def test_insufficient_psu(self):
        ...
        self.assertFalse(
            report["compatible"], "weak PSU should not be compatible with high-end GPU"
        )
        self.assertEqual(
            report["status"],
            "incompatible",
            "status should be incompatible for PSU failure",
        )
        self.assertTrue(
            any(
                "insufficient" in issue["message"].lower() for issue in report["issues"]
            ),
            "should report insufficient wattage",
        )
    ...
    def test_gpu_too_large_for_case(self):
        ...
        self.assertFalse(
            report["compatible"], "large GPU should not fit the compact case"
        )
        self.assertEqual(
            report["status"],
            "incompatible",
            "status should be incompatible for size mismatch",
        )
        self.assertTrue(
            any(
                "GPU" in issue["component"] and "case" in issue["message"].lower()
                for issue in report["issues"]
            ),
            "should report GPU/case clearance problem",
        )

    def test_resolution_for_midrange_gpu_is_conservative(self):
        self.assertEqual(
            PerformanceAnalyzer.resolution(58), "1080p Ultra / 1440p High (60+ fps)"
        )
    ...
    def test_new_components_available(self):
        ...
        ddr4_3600 = next(
            (c for c in components if c.get("name") == "32GB DDR4 3600MHz"), None
        )
        ...
        self.assertEqual(rtx_4070.get("price"), 43795.0)
        self.assertEqual(ddr4_3600.get("price"), 13950.0)
