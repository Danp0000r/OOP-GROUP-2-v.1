"""
Questionnaire service — recommends a complete build based on user answers
from the AI PC Builder questionnaire.
"""
import json
...
def get_build_recommendation(answers: dict) -> dict:
    """
    Given questionnaire answers, return a recommended full build.
    
    NEW APPROACH: Generate all possible compatible builds first, find min/max prices,
    then dynamically categorize as low/normal/high instead of using fixed budget ranges.
    
    answers should contain:
    - targetTier: 'Education, Office & Light Gaming' | 'Pure Gaming Focus' | 'Streaming, Video Editing & Creation'
    - budget: 'Low Cost' | 'Normal Cost' | 'High Cost' (or old format will map automatically)
    - formFactor: 'Micro-ATX (Compact Layout)' | 'Standard ATX (Spacious Layout)'
    - style: 'Stealth Air Cooling (No-RGB)' | 'Brilliant Liquid RGB Performance'
    - platform: 'Team AMD & DDR4 Memory' | 'Team Intel & DDR5 Memory' | 'No Preference (Optimized Value)'
    
    Returns a dict with component_ids and metadata about the recommended build.
    """
...
def _get_allocations(tier: str) -> dict:
    """Get scenario-aware budget allocations."""
...
def _categorize_builds_by_price(all_builds: list) -> dict:
    """
    Categorize generated builds into low/normal/high price tiers.
    
    Uses price percentiles:
    - Low: bottom 33%
    - Normal: middle 34-67%
    - High: top 33%
    """
...
def _generate_all_candidate_builds(tier: str, allocations: dict,
    ...
    """
    Generate ALL possible compatible builds without fixed budget constraints.
    
    Strategy:
    1. Get CPU candidates at multiple performance levels (budget/mid/high)
    2. For each CPU, build complete systems with min/mid/max component choices
    3. Validate each via CompatibilityService
    4. Rank by compatibility status + performance, return all viable builds
    """
    all_builds = []
...
def _build_around_cpu_variant(cpu_comp: Component, tier: str, allocations: dict,
    ...
    """
    Build a complete system around a CPU with variant selection:
    - 'min': cheapest compatible parts
    - 'mid': moderate price parts
    - 'max': best/most expensive compatible parts
    """
...
def _generate_candidate_builds(tier: str, avg_budget: float, allocations: dict,
    ...
    """
    Generate multiple candidate builds by:
    1. Creating top CPU candidates with dynamic budget adjustment
    2. For each CPU, finding compatible motherboards and building upward
    3. Evaluating each complete build via CompatibilityService
    4. Ranking by compatibility status + performance score
    5. Returning top compatible builds (or best-effort if all have warnings)
    """
    all_builds = []
...
def _build_around_cpu(cpu_comp: Component, tier: str, avg_budget: float, allocations: dict,
                       form_factor: str, style: str,
                       prefer_amd: bool, prefer_intel: bool) -> list:
    """Build complete systems around a chosen CPU with compatibility validation and dynamic budget."""
    builds = []
...
def _compute_scenario_score(tier: str, cpu_s: int, gpu_s: int) -> int:
    """Compute scenario-aware performance score."""
    if 'Streaming' in tier or 'Creation' in tier:
...
def _find_compatible_motherboards(cpu_comp: Component, form_factor: str, budget: int, limit: int = 3) -> list:
    """Find motherboards compatible with a given CPU (by socket)."""
...
def _find_motherboards(form_factor: str, budget: int, limit: int = 3) -> list:
    """Find motherboards matching form factor with fallback to any."""
    q = Component.query.filter_by(category='Motherboard').filter(Component.price <= budget)
...
def _find_compatible_ram(mb_comp: Component, budget: int, limit: int = 2) -> list:
    """Find RAM compatible with motherboard memory type with fallback."""
...
def _find_ram(budget: int, limit: int = 2) -> list:
    """Find RAM without memory type constraints."""
    q = Component.query.filter_by(category='RAM').filter(Component.price <= budget)
    return q.order_by(Component.performance_score.desc()).limit(limit).all()


def _select_gpu_for_scenario(tier: str, budget: int) -> Component:
    """Select GPU optimized for scenario."""
...
def _select_cpu(tier: str, budget: int, prefer_amd: bool = False, prefer_intel: bool = False) -> Component:
    """Select a CPU based on tier and budget."""
...
def _candidate_cpus(tier: str, budget: int, prefer_amd: bool, prefer_intel: bool, limit: int = 6) -> list:
    """Get top CPU candidates ordered by performance score."""
    q = Component.query.filter_by(category='CPU').filter(Component.price <= budget)
    if prefer_amd:
        q = q.filter(or_(Component.name.ilike('%ryzen%'), Component.name.ilike('%amd%')))
    elif prefer_intel:
...
def _select_storage(budget: int) -> Component:
    """Select storage based on budget with fallback."""
...
def _select_cooling(style: str, budget: int) -> Component:
    """Select cooling solution based on style preference with fallback."""
...
def _select_psu(budget: int) -> Component:
    """Select PSU based on budget with fallback."""
...
def _select_case(form_factor: str, budget: int) -> Component:
    """Select case based on form factor and budget with fallback."""
...
def _select_psu_for_system(system_parts: dict, budget: int) -> Component:
    """Select PSU with appropriate wattage for system."""
    q = Component.query.filter_by(category='PSU').filter(Component.price <= budget)
    comp = q.order_by(Component.price.desc()).first()
