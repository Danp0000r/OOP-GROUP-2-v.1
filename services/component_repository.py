...
class ComponentRepository:
    ...
    def get_by_price_range(
        ...
        components = Component.query.filter(
            Component.price >= min_price, Component.price <= max_price
        ).all()
        return [ComponentFactory.create(c) for c in components]

    def get_by_spec(
        self, category: str, spec_key: str, spec_value: Any
    ) -> List[BaseComponent]:
        components = self.get_by_category(category)
        return [c for c in components if c.specs.get(spec_key) == spec_value]
