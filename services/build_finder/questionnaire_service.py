...
def get_build_recommendation(answers: dict) -> dict:
    """Generate a PC build recommendation based on questionnaire answers."""
    ...
    all_builds = _generate_all_candidate_builds(
        tier, allocations, form_factor, style, prefer_amd, prefer_intel
    )
    ...
    if not all_builds:
        return {
            ...
            "answers": answers,
            "issues": [
                "Unable to assemble any compatible builds. Database may lack components."
            ],
        }
    ...
    if any(token in budget_lower for token in ["low", "budget build", "under"]):
        ...
        token in budget_lower for token in ["high", "high performance", "enthusiast"]
    ):
        selected_build = builds_by_tier.get("high", [None])[
            -1
    ...
    # Pass full component dicts (not stripped) so fixer can access compatibility, specs, etc.
    parts_for_check = components[:]  # Use full component dicts with all fields

    # Check actual compatibility (not just the stored report)
    report = CompatibilityService.evaluate_build(parts_for_check)
    changes_made = []

    # If not fully compatible, attempt minimal fixes while respecting questionnaire answers.
    # This includes 'warning' builds so known compatibility issues can still be corrected.
    if report.get("status") != "compatible":
        fix_result = BuildFixService.fix_build(parts_for_check, answers=answers)
        # Use best-effort fixed components even if 'fixed' is False (partial fixes are better than nothing)
        if fix_result.get("components"):
            ...
            for comp in fixed_components:
                ...
                if comp.get("id"):
                    ...
                    db_comp = Component.query.filter_by(
                        name=comp.get("name"), category=comp.get("category")
                    ).first()
                    if db_comp:
                        recommended_ids.append(db_comp.component_id)

            total_price = int(
                round(sum((c.get("price") or 0) for c in fixed_components))
            )
...
def _generate_all_candidate_builds(
    tier: str,
    allocations: dict,
    form_factor: str,
    style: str,
    ...
    budget_cpus = _candidate_cpus(
        tier, 6000, prefer_amd, prefer_intel, limit=2
    )  # Budget CPUs
    mid_cpus = _candidate_cpus(
        tier, 9000, prefer_amd, prefer_intel, limit=3
    )  # Mid CPUs
    high_cpus = _candidate_cpus(
        tier, 12000, prefer_amd, prefer_intel, limit=2
    )  # High CPUs
    ...
    def build_score(build):
        ...
        status_priority = {"compatible": 3, "warning": 2, "incompatible": 1}.get(
            status, 0
        )
...
def _build_around_cpu_variant(
    ...
    if variant == "min":
        mb_candidates = _find_compatible_motherboards(
            cpu_comp, form_factor, 3000, limit=1
        )
    elif variant == "max":
        mb_candidates = _find_compatible_motherboards(
            cpu_comp, form_factor, 10000, limit=2
        )
    else:
        mb_candidates = _find_compatible_motherboards(
            cpu_comp, form_factor, 5000, limit=2
        )
    ...
    if "Gaming" in tier:
        if variant == "min":
            gpu_candidates = (
                Component.query.filter_by(category="GPU")
                .filter(Component.price <= 10000)
                .order_by(Component.performance_score.desc())
                .limit(1)
                .all()
            )
        elif variant == "max":
    ...
    gpu_dict = (
        gpu_comp.to_dict()
        if gpu_comp
        else {"id": 0, "name": "No GPU", "price": 0, "category": "GPU"}
    )
    ...
    if variant == "min":
        storage_candidates = (
            Component.query.filter_by(category="Storage")
            .order_by(Component.price.asc())
            .limit(1)
            .all()
        )
    ...
    storage_dict = (
        storage_comp.to_dict()
        if storage_comp
        else {"id": 0, "name": "No Storage", "price": 0, "category": "Storage"}
    )
    ...
    if "Stealth" in style:
        cooling_candidates = (
            Component.query.filter_by(category="Cooling")
            .filter(Component.name.like("%Stock%"))
            .limit(1)
            .all()
        )
    ...
    cooling_dict = (
        cooling_comp.to_dict()
        if cooling_comp
        else {"id": 0, "name": "Stock Cooler", "price": 0, "category": "Cooling"}
    )
    ...
    if variant == "min":
        psu_candidates = (
            Component.query.filter_by(category="PSU")
            .order_by(Component.price.asc())
            .limit(1)
            .all()
        )
    ...
    psu_dict = (
        psu_comp.to_dict()
        if psu_comp
        else {"id": 0, "name": "Generic PSU", "price": 2595, "category": "PSU"}
    )
    ...
    if variant == "min":
        case_candidates = (
            Component.query.filter_by(category="Case")
            .order_by(Component.price.asc())
            .limit(1)
            .all()
        )
    ...
    case_dict = (
        case_comp.to_dict()
        if case_comp
        else {"id": 0, "name": "Generic Case", "price": 1550, "category": "Case"}
    )
...
def _generate_candidate_builds(
    ...
    compatible_builds = [
        b for b in all_builds if b.get("_report", {}).get("status") == "compatible"
    ]
    if compatible_builds:
        return compatible_builds[:5]

    warning_builds = [
        b for b in all_builds if b.get("_report", {}).get("status") == "warning"
    ]
    if warning_builds:
...
def _build_around_cpu(
    ...
    mb_candidates = _find_compatible_motherboards(
        cpu_comp, form_factor, mb_budget, limit=3
    )
    ...
    if not mb_candidates:
        ...
        mb_candidates = _find_compatible_motherboards(
            cpu_comp, form_factor, relaxed_mb_budget, limit=3
        )
    ...
    for mb_comp in mb_candidates:
        ...
        if not ram_candidates:
            ram_candidates = _find_compatible_ram(
                mb_comp, int(ram_budget * 1.3), limit=3
            )
        ...
        for ram_comp in ram_candidates:
            ...
            # Storage
            storage_budget = int(
                remaining4 * remaining_allocations.get("Storage", 0.10)
            )
            ...
            # Cooling
            cooling_budget = int(
                remaining5 * remaining_allocations.get("Cooling", 0.05)
            )
            ...
            psu_comp = _select_psu_for_system(
                {"CPU": cpu_dict, "GPU": gpu_dict}, psu_budget
            )

            if not psu_comp:
                psu_comp = _select_psu_for_system(
                    {"CPU": cpu_dict, "GPU": gpu_dict}, int(psu_budget * 1.5)
                )
...
def _find_compatible_motherboards(
    cpu_comp: Component, form_factor: str, budget: int, limit: int = 3
) -> list:
    """Find motherboards compatible with CPU socket, using Python-side filtering."""
    q = Component.query.filter_by(category="Motherboard").filter(
        Component.price <= budget
    )

    # Get all candidate motherboards
    all_candidates = q.order_by(Component.performance_score.desc()).limit(50).all()

    # Extract socket from CPU
    cpu_socket = None
    try:
        specs = cpu_comp.specs or {}
        compat = cpu_comp.compatibility or {}
        cpu_socket = compat.get("motherboard_socket") or specs.get("socket")
        if cpu_socket:
            cpu_socket = str(cpu_socket).upper()
    except Exception:
        pass

    # Filter motherboards by socket using Python-side matching
    matching = []
    for mb in all_candidates:
        mb_specs = mb.specs or {}
        mb_compat = mb.compatibility or {}
        mb_socket = (
            mb_compat.get("cpu_socket")
            or mb_specs.get("socket_type")
            or mb_specs.get("socket")
        )

        if mb_socket:
            mb_socket = str(mb_socket).upper()

        # If CPU has a socket requirement, match it
        if cpu_socket and mb_socket:
            if cpu_socket == mb_socket:
                matching.append(mb)
        elif not cpu_socket:
            # No CPU socket requirement, accept any MB
            matching.append(mb)

    # Return matching motherboards up to limit
    return matching[:limit] if matching else all_candidates[:limit]
...
def _find_compatible_ram(mb_comp: Component, budget: int, limit: int = 2) -> list:
    """Find RAM compatible with motherboard type, using Python-side filtering."""
    q = Component.query.filter_by(category="RAM").filter(Component.price <= budget)

    # Get all candidate RAM
    all_candidates = q.order_by(Component.performance_score.desc()).limit(50).all()

    # Extract RAM type requirement from motherboard
    mb_ram_type = None
    try:
        specs = mb_comp.specs or {}
        compat = mb_comp.compatibility or {}
        mb_ram_type = compat.get("ram_type") or specs.get("ram_type")
        if mb_ram_type:
            mb_ram_type = str(mb_ram_type).upper()
    except Exception:
        pass

    # Filter RAM by type using Python-side matching
    matching = []
    for ram in all_candidates:
        ram_specs = ram.specs or {}
        ram_compat = ram.compatibility or {}
        ram_type = ram_compat.get("ram_type") or ram_specs.get("type")

        if ram_type:
            ram_type = str(ram_type).upper()

        # If MB has a RAM type requirement, match it
        if mb_ram_type and ram_type:
            if mb_ram_type == ram_type:
                matching.append(ram)
        elif not mb_ram_type:
            # No MB RAM type requirement, accept any RAM
            matching.append(ram)

    # Return matching RAM up to limit
    return matching[:limit] if matching else all_candidates[:limit]
...
def _select_cpu(
    tier: str, budget: int, prefer_amd: bool = False, prefer_intel: bool = False
) -> Component:
    q = Component.query.filter_by(category="CPU").filter(Component.price <= budget)

    if prefer_amd:
        q = q.filter(
            or_(Component.name.ilike("%ryzen%"), Component.name.ilike("%amd%"))
        )
    elif prefer_intel:
        q = q.filter(
            or_(Component.name.ilike("%intel%"), Component.name.ilike("%core%"))
        )
...
def _candidate_cpus(
    tier: str, budget: int, prefer_amd: bool, prefer_intel: bool, limit: int = 6
) -> list:
    q = Component.query.filter_by(category="CPU").filter(Component.price <= budget)
    if prefer_amd:
        q = q.filter(
...
def _select_storage(budget: int) -> Component:
    ...
    if not comp:
        q = Component.query.filter_by(category="Storage").filter(
            Component.price <= int(budget * 1.5)
        )
        comp = q.order_by(Component.performance_score.desc()).first()
...
def _select_cooling(style: str, budget: int) -> Component:
    ...
    if "Stealth" in style or "Air" in style:
        ...
        air_cooling = (
            q.filter(~Component.name.ilike("%liquid%"))
            .order_by(Component.price.desc())
            .first()
        )
        if air_cooling:
        ...
        liquid = (
            q.filter(
                or_(Component.name.ilike("%liquid%"), Component.name.ilike("%aio%"))
            )
            .order_by(Component.price.desc())
            .first()
        )
...
def _select_psu(budget: int) -> Component:
    ...
    # Fallback: relax budget
    if not comp:
        q = Component.query.filter_by(category="PSU").filter(
            Component.price <= int(budget * 1.5)
        )
        comp = q.order_by(Component.price.desc()).first()
