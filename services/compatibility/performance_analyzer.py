class PerformanceAnalyzer:
    ...
    def upgrades(gpu_s, cpu_s=None, has_gpu=False, ram_capacity=0, psu_w=0):
        ...
        if gpu_s >= 75:
            suggestions.append(
                "GPU already high-end. Consider 32GB RAM or 1440p+ monitor upgrade."
            )
        elif gpu_s >= 58:
            suggestions.append(
                "GPU solid for 1080p. Next tier: RTX 4070 / RX 7800 XT for 1440p."
            )
        elif gpu_s >= 48:
            suggestions.append(
                "GPU good for 1080p. Consider RTX 4060 / RX 6700 for improved 1440p gaming."
            )
        elif gpu_s >= 38:
            suggestions.append(
                "Upgrade GPU to RTX 4060 / RX 6600 for smoother 1080p High settings."
            )
        else:
            suggestions.append(
                "Upgrade GPU to at least GTX 1660 Super / RX 580 for playable 1080p gaming."
            )
        ...
        if cpu_s and gpu_s > 0:
            ...
            if ratio < 0.6:
                suggestions.append(
                    "IMPORTANT: CPU is bottlenecking GPU. Upgrade CPU to match GPU tier."
                )
            elif ratio > 1.5:
                suggestions.append(
                    "CPU upgrade not priority. GPU upgrade will provide more gaming benefit."
                )

        # PSU headroom warning
        if psu_w > 0 and gpu_s >= 58 and psu_w < 750:
            suggestions.append(
                "PSU marginal for high-end GPU upgrades. Consider 750W+ PSU."
            )

        # RAM adequacy
        if ram_capacity > 0 and ram_capacity < 16:
            suggestions.append(
                "Upgrade to 16GB RAM for modern games (8GB is below recommended)."
            )
