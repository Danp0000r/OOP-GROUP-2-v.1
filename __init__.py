...
def create_app():
    ...
    app.config["SQLALCHEMY_BINDS"] = {
        "components": "sqlite:///"
        + os.path.join(instance_dir, "components.db").replace("\\", "/")
    }
    ...
    update_db_path = os.path.join(
        os.path.dirname(__file__), "data reset", "update_db.py"
    )
    spec = importlib.util.spec_from_file_location(
        "data_reset_update_db", update_db_path
    )
    if not spec or not spec.loader:
        raise ImportError(
            f"Unable to load database update module from {update_db_path}"
        )
    ...
    def nexus3d_positions():
        ...
        try:
            ...
            return (
                jsonify({"error": "Failed to save positions.", "detail": str(e)}),
                500,
            )
